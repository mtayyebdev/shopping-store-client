...
import { getOrderData, getOrders, orderPayment } from "../store/publicSlices/OrderSlice.jsx";
...
export default function Payment() {
  ...
  const handlePayment = async (e) => {
    ...
    if (selectedMethod === "card") {
      ...
      if (!upiId) {
        toast.error("Please enter UPI ID or PayPal email");
        return;
      }
    }

    setIsProcessing(true);
    await disptach(orderPayment({ orderId: orderid, paymentMethod: selectedMethod }))
      .then((res) => {
        if (res.type === "orderpayment/fulfilled") {
  ...
  const patchOrderData = async (orderid) => {
    await disptach(getOrderData(orderid))
      .then((res) => {
        if (res.type === "getorderdata/fulfilled") {
          setorderDetails(res.payload.data);
        }
      })
  }
  useEffect(() => {
    patchOrderData(orderid);
  }, [orderid])
              ...
              {paymentMethods.map((method) => {
                ...
                return (
                  <div
                    ...
                    className={`border-2 rounded-lg p-4 cursor-pointer transition-all duration-300 ${selectedMethod === method.id
                      ? "border-btn2 bg-btn2/10"
                      : "border-gray-200 hover:border-gray-300"
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedMethod === method.id ? "bg-btn2" : "bg-gray-100"
                        }`}>
                        <Icon className={`w-5 h-5 ${selectedMethod === method.id ? "text-white" : "text-text1"
                          }`} />
                      </div>
          ...
          <div className="bg-primary rounded p-4 sticky top-4">
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              <p className="text-xs text-gray-700">
                <span className="font-medium">{orderDetails?.shippingAddress?.username}</span>
                <br />
            ...
            <div className="space-y-2 mb-4 pb-4 border-b border-gray-200">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-text2">
                  Rs.{orderDetails?.itemsPrice.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text1">Delivery Charges</span>
                <span className={`font-semibold ${orderDetails?.shippingPrice === 0 ? 'text-green-600' : 'text-text2'}`}>
                  {orderDetails?.shippingPrice === 0 ? 'FREE' : `Rs.${orderDetails?.shippingPrice}`}
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between mb-6">
              ...
              <span className="text-xl font-bold text-text2">
                Rs.{orderDetails?.totalPrice.toLocaleString()}
              </span>
            </div>
            ...
            >
              {isProcessing ? (
                ...
                `Pay Rs.${orderDetails?.totalPrice.toLocaleString()}`
              )}
            </button>
